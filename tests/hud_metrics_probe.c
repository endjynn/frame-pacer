...
int main(int argc, char **argv)
{
    ...
    printf("available=0x%x cpu=%u cpu_temp=%u gpu=%u gpu_temp=%u\n", snapshot.available,
           snapshot.cpu_use_percent, snapshot.cpu_temp_celsius,
           snapshot.gpu_use_percent, snapshot.gpu_temp_celsius);
    frame_pacer_metrics_destroy(&metrics);
    return (snapshot.available & (FRAME_PACER_METRIC_GPU_USE | FRAME_PACER_METRIC_GPU_TEMP)) ==
           (FRAME_PACER_METRIC_GPU_USE | FRAME_PACER_METRIC_GPU_TEMP) ? 0 : 1;
}
