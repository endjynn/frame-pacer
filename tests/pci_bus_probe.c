...
static int has_pci_extension(VkPhysicalDevice device)
{
    ...
    if (vkEnumerateDeviceExtensionProperties(device, 0, &count, extensions) !=
        VK_SUCCESS)
        return 0;
...
int main(void)
{
    ...
    VkInstanceCreateInfo create = {.sType =
                                       VK_STRUCTURE_TYPE_INSTANCE_CREATE_INFO,
                                   .pApplicationInfo = &app};
    VkInstance instance;
    VkPhysicalDevice devices[16];
    uint32_t count = 16, index;
    if (vkCreateInstance(&create, 0, &instance) != VK_SUCCESS)
        return 1;
    ...
    for (index = 0; index < count; ++index) {
        ...
        printf("vendor=0x%04x device=0x%04x name=%s pci=%u:%u:%u.%u\n",
               ...
               properties.properties.deviceName, pci.pciDomain, pci.pciBus,
