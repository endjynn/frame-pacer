...
prefix="$root/build/winepath-probe-prefix"
path=$(printf '%s' "$root/build/windows/x86_64" | sed 's|/|\\\\|g; s|^|Z:|')
WINEPREFIX="$prefix" WINEARCH=win64 WINEDEBUG=-all WINEPATH="$path" "$wine" "$root/build/windows/probe-client/frame_pacer_probe.exe"
