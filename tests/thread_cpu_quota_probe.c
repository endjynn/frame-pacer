...
#define LOAD(member, name)                                                     \
    if (!symbol(library, name, &api->member, sizeof(api->member)))             \
    goto fail
...
static bool scope_name(char *output, size_t size)
{
    ...
    int written;
    if (!file || !fgets(boot_id, sizeof(boot_id), file)) {
        if (file)
            fclose(file);
        return false;
    }
    (void)fclose(file);
    char *ending = strpbrk(boot_id, "\r\n");
    if (ending)
        *ending = '\0';
    written = snprintf(output, size,
                       "frame-pacer-thread-probe-u%ju-b%.12s-p%ju.scope",
                       (uintmax_t)getuid(), boot_id, (uintmax_t)getpid());
    return written > 0 && (size_t)written < size;
}

static bool property_prefix(const struct sd_api *api,
...
static bool property_bool(const struct sd_api *api,
                          struct sd_bus_message *message, const char *name,
                          int value)
{
    return property_prefix(api, message, name, "b") &&
           api->message_append(message, "b", value) >= 0 &&
           api->message_close_container(message) >= 0 &&
           api->message_close_container(message) >= 0;
}

static bool property_string(const struct sd_api *api,
                            struct sd_bus_message *message, const char *name,
                            const char *value)
{
    return property_prefix(api, message, name, "s") &&
           api->message_append(message, "s", value) >= 0 &&
           api->message_close_container(message) >= 0 &&
           api->message_close_container(message) >= 0;
}

static bool property_pid(const struct sd_api *api,
...
static bool start_scope(struct sd_api *api, struct sd_bus *bus,
                        const char *name)
{
    ...
    if (api->message_new_method_call(bus, &request, "org.freedesktop.systemd1",
        ...
        !property_pid(api, request) ||
        !property_bool(api, request, "Delegate", 1) ||
        ...
        api->message_close_container(request) < 0 ||
        api->bus_call(bus, request, PROBE_TIMEOUT_US, &error, &reply) < 0)
        goto done;
    result = true;
done:
    ...
    for (attempt = 0; attempt < 50; ++attempt) {
        ...
        if (file && fgets(line, sizeof(line), file) &&
            ...
            if (terminal && !strcmp(terminal + 1, scope)) {
                (void)fclose(file);
                written = snprintf(output, size, "/sys/fs/cgroup%s", line + 3);
                return written >= 0 && (size_t)written < size;
            }
        }
        if (file)
...
static bool cgroup_file(char *output, size_t size, const char *root,
                        const char *child, const char *file)
{
    int written =
        snprintf(output, size, "%s%s%s%s%s", root, child ? "/" : "",
...
static bool topology_stage(const char *scope, const char **failed_stage,
                           int *failed_errno)
{
    char root[1400], path[1500];
    bool scope_cpu_enabled = false, root_created = false, left_created = false,
         right_created = false;
    bool result = false;

    *failed_stage = "construct threads root";
    *failed_errno = 0;
    if (!cgroup_file(root, sizeof(root), scope, "threads", 0))
        goto done;
    ...
    *failed_stage = "create left child";
    if (!cgroup_file(path, sizeof(path), root, "left", 0) ||
        mkdir(path, 0700)) {
        *failed_errno = errno;
        goto done;
    }
    ...
    *failed_stage = "create right child";
    if (!cgroup_file(path, sizeof(path), root, "right", 0) ||
        mkdir(path, 0700)) {
        *failed_errno = errno;
        goto done;
    }
    ...
    *failed_stage = "move probe TID into left child";
    ...
    if (!cgroup_file(path, sizeof(path), scope, 0, "cgroup.threads") ||
        !write_own_tid(path)) {
        result = false;
        *failed_stage = "return probe to delegated scope";
        *failed_errno = errno;
    }
    if (right_created && cgroup_file(path, sizeof(path), root, "right", 0) &&
        rmdir(path)) {
        result = false;
        *failed_stage = "remove right child";
        *failed_errno = errno;
    }
    if (left_created && cgroup_file(path, sizeof(path), root, "left", 0) &&
        rmdir(path)) {
        result = false;
    ...
    if (scope_cpu_enabled) {
        if (!cgroup_file(path, sizeof(path), scope, 0,
...
/* Applies temporary ceilings only below the verified scope and removes them
 * before return. */
static bool quota_stage(const char *scope, const char **failed_stage,
                        int *failed_errno, uint64_t *fifty_left,
                        uint64_t *fifty_right, uint64_t *seventy_five_left,
                        uint64_t *seventy_five_right)
{
    ...
    *failed_errno = 0;
    if (!cgroup_file(root, sizeof(root), scope, "quota-threads", 0))
        goto done;
    ...
    *failed_stage = "mark quota right child threaded";
    ...
    if (!cgroup_file(left.threads_path, sizeof(left.threads_path), root, "left",
                     "cgroup.threads") ||
        !cgroup_file(right.threads_path, sizeof(right.threads_path), root,
                     "right", "cgroup.threads") ||
        !cgroup_file(left_stat, sizeof(left_stat), root, "left", "cpu.stat") ||
        !cgroup_file(right_stat, sizeof(right_stat), root, "right", "cpu.stat"))
        goto done;
    *failed_stage = "start quota workers";
    if (pthread_create(&left_thread, 0, burn_worker, &left)) {
        *failed_errno = errno;
        goto done;
    }
    left_started = true;
    if (pthread_create(&right_thread, 0, burn_worker, &right)) {
        *failed_errno = errno;
        goto done;
    }
    right_started = true;
    *failed_stage = "move quota worker TIDs";
    if (!wait_workers(&left, &right)) {
        *failed_errno = errno;
        goto done;
    }
    *failed_stage = "apply 50 percent ceilings";
    if (!cgroup_file(path, sizeof(path), root, "left", "cpu.max") ||
        !write_text(path, "50000 100000") ||
        !cgroup_file(path, sizeof(path), root, "right", "cpu.max") ||
        !write_text(path, "50000 100000")) {
        *failed_errno = errno;
        goto done;
    }
    if (!cpu_usage_usec(left_stat, &left_before) ||
        !cpu_usage_usec(right_stat, &right_before))
        goto done;
    atomic_store_explicit(&left.start, true, memory_order_release);
    atomic_store_explicit(&right.start, true, memory_order_release);
    if (!sleep_seconds(WINDOW_SECONDS) ||
        !cpu_usage_usec(left_stat, &left_after) ||
        !cpu_usage_usec(right_stat, &right_after))
        goto done;
    *fifty_left = left_after - left_before;
    *fifty_right = right_after - right_before;
    *failed_stage = "apply 75 percent ceilings";
    if (!cgroup_file(path, sizeof(path), root, "left", "cpu.max") ||
        !write_text(path, "75000 100000") ||
        !cgroup_file(path, sizeof(path), root, "right", "cpu.max") ||
        !write_text(path, "75000 100000")) {
        *failed_errno = errno;
        goto done;
    ...
    *failed_stage = "validate sampled ceilings";
    if (*fifty_left > WINDOW_USEC * 5 / 8 ||
        *fifty_right > WINDOW_USEC * 5 / 8 ||
        *fifty_left + *fifty_right <= WINDOW_USEC * 3 / 4 ||
        *seventy_five_left > WINDOW_USEC * 7 / 8 ||
        *seventy_five_right > WINDOW_USEC * 7 / 8 ||
        *seventy_five_left <= *fifty_left * 11 / 10 ||
        *seventy_five_right <= *fifty_right * 11 / 10)
        goto done;
    result = true;
done:
     ...
     */
    if (left_created &&
        cgroup_file(path, sizeof(path), root, "left", "cpu.max") &&
        !write_text(path, "max 100000")) {
        result = false;
        *failed_stage = "clear left ceiling";
        *failed_errno = errno;
    }
    if (right_created &&
        cgroup_file(path, sizeof(path), root, "right", "cpu.max") &&
        !write_text(path, "max 100000")) {
        result = false;
        *failed_stage = "clear right ceiling";
        *failed_errno = errno;
    }
    ...
    atomic_store_explicit(&left.start, true, memory_order_release);
    atomic_store_explicit(&right.start, true, memory_order_release);
    if (right_started && pthread_join(right_thread, 0)) {
        result = false;
        *failed_stage = "join right worker";
        *failed_errno = errno;
    }
    if (left_started && pthread_join(left_thread, 0)) {
        result = false;
        *failed_stage = "join left worker";
        *failed_errno = errno;
    }
    if (right_created && cgroup_file(path, sizeof(path), root, "right", 0) &&
        rmdir(path)) {
        result = false;
        *failed_stage = "remove quota right child";
        *failed_errno = errno;
    }
    if (left_created && cgroup_file(path, sizeof(path), root, "left", 0) &&
        rmdir(path)) {
        result = false;
        *failed_stage = "remove quota left child";
        *failed_errno = errno;
    }
    if (root_created && rmdir(root)) {
        result = false;
        *failed_stage = "remove quota threads root";
        *failed_errno = errno;
    }
    if (scope_cpu_enabled &&
        (!cgroup_file(path, sizeof(path), scope, 0, "cgroup.subtree_control") ||
         !write_text(path, "-cpu"))) {
        result = false;
        *failed_stage = "disable CPU controller below quota scope";
        *failed_errno = errno;
    }
    return result;
}
...
int main(void)
{
    ...
    int failed_errno;
    uint64_t fifty_left = 0, fifty_right = 0, seventy_five_left = 0,
             seventy_five_right = 0;

    if (!getenv("FRAME_PACER_THREAD_CPU_QUOTA_INTEGRATION")) {
        fputs("thread-cpu-quota-probe: set "
              "FRAME_PACER_THREAD_CPU_QUOTA_INTEGRATION=1 to run\n",
              stderr);
        return 77;
    }
    if (!host_pid_visible()) {
    ...
    if (!api_open(&api) || api.open_user(&bus) < 0 ||
        !scope_name(name, sizeof(name)) || !start_scope(&api, bus, name) ||
        !cgroup_path(path, sizeof(path), name)) {
        if (bus)
            api.bus_unref(bus);
        api_close(&api);
        fputs("thread-cpu-quota-probe: delegated scope unavailable; no "
              "threaded topology attempted\n",
              stderr);
        return 77;
    }
    if (!topology_stage(path, &failed_stage, &failed_errno)) {
        ...
        fprintf(
            stderr,
            "thread-cpu-quota-probe: threaded topology unavailable at %s%s%s; "
            "rolled back owned paths\n",
            failed_stage, failed_errno ? ": " : "",
            failed_errno ? strerror(failed_errno) : "");
        return 77;
    }
    if (!quota_stage(path, &failed_stage, &failed_errno, &fifty_left,
                     &fifty_right, &seventy_five_left, &seventy_five_right)) {
        api.bus_unref(bus);
        api_close(&api);
        fprintf(stderr,
                "thread-cpu-quota-probe: quota stage unavailable at %s%s%s; "
    ...
    printf("scope=%s cgroup=%s thread-topology=confirmed "
           ...
           name, path, fifty_left, fifty_right, seventy_five_left,
           seventy_five_right);
    api.bus_unref(bus);
    api_close(&api);
    return 0;
