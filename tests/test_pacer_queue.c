...
int main(void)
{
    ...
    assert(!frame_pacer_queue_needs_fallback(
        &first, 100 + FRAME_PACER_PRESENT_QUIET_NS, &entered, &submits));
    assert(submits == 1);
    assert(frame_pacer_queue_needs_fallback(
        &first, 101 + FRAME_PACER_PRESENT_QUIET_NS, &entered, &submits));
    assert(entered && submits == 2 && first.fallback_active);
    assert(frame_pacer_queue_needs_fallback(
        &first, 102 + FRAME_PACER_PRESENT_QUIET_NS, &entered, &submits));
    assert(!entered && submits == 3);
    frame_pacer_queue_note_present(&first);
    assert(!first.fallback_active && first.submits_since_present == 0);
    assert(
        !frame_pacer_queue_needs_fallback(&second, 1000, &entered, &submits));
