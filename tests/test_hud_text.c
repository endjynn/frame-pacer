...
int main(void)
{
    ...
    frame_pacer_hud_text_format(&text,
                                &(struct frame_pacer_metrics_snapshot){0}, 0, 0,
                                70, true, false, 75);
    assert(!strcmp(text.lines[0], "GPU  N/A  N/A"));
    assert(!strcmp(text.lines[2], "THR  N/A  N/A"));
    assert(!strcmp(text.lines[3], "FPS  N/A  70\x7e"));
    frame_pacer_hud_text_format(&text, &metrics, 1, 999, 999, false, false, 0);
    assert(!strcmp(text.lines[2], "FPS 999\x7e 999\x7e"));
    frame_pacer_hud_text_format(&text, 0, 1, 1000, 1000, true, true, 101);
    assert(!strcmp(text.lines[0], "GPU  N/A  N/A"));
    assert(!strcmp(text.lines[2], "THR  N/A  N/A"));
