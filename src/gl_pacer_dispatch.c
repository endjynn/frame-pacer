...
static void gl_function(struct frame_pacer_gl_dispatch *dispatch,
                        void *library, const char *name, void *function,
                        size_t size)
{
    void *symbol = dispatch->real_dlsym ?
        dispatch->real_dlsym(library, name) : 0;
                              ...
                              void *library)
{
    ...
    return (dispatch->next_glx_current_context ||
           ...
           dispatch->gl_blend_equation_separate &&
           dispatch->gl_is_enabled && dispatch->gl_active_texture &&
           dispatch->gl_bind_texture && dispatch->gl_bind_sampler &&
           dispatch->gl_bind_framebuffer && dispatch->gl_viewport &&
           dispatch->gl_scissor && dispatch->gl_color_mask &&
           dispatch->gl_create_shader && dispatch->gl_shader_source &&
           dispatch->gl_compile_shader && dispatch->gl_get_shader_iv &&
           dispatch->gl_get_shader_info_log && dispatch->gl_delete_shader &&
           dispatch->gl_create_program && dispatch->gl_attach_shader &&
           dispatch->gl_bind_attrib_location && dispatch->gl_link_program &&
           dispatch->gl_get_program_iv &&
           dispatch->gl_get_program_info_log &&
           dispatch->gl_delete_program && dispatch->gl_use_program &&
           dispatch->gl_get_uniform_location && dispatch->gl_uniform_2f &&
           dispatch->gl_gen_vertex_arrays &&
...
#define LOAD_NEXT_IF_MISSING(field, name)                                 \
    do {                                                                  \
        if (!dispatch->field) LOAD_FROM(RTLD_NEXT, field, (name));         \
    } while (0)
...
bool frame_pacer_gl_dispatch_init(struct frame_pacer_gl_dispatch *dispatch)
{
    ...
    if (dispatch->egl_reference) {
        if (!dispatch->next_egl_swap)
            LOAD_FROM(dispatch->egl_reference, next_egl_swap,
                      "eglSwapBuffers");
...
{
    if (!dispatch) return;
    if (dispatch->egl_reference) (void)dlclose(dispatch->egl_reference);
    if (dispatch->gl_reference) (void)dlclose(dispatch->gl_reference);
    if (dispatch->libc_reference) (void)dlclose(dispatch->libc_reference);
    memset(dispatch, 0, sizeof(*dispatch));
}
