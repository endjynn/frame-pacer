...
#define FRAME_PACER_VULKAN_HUD_INITIALIZER(registry_value, context_value, \
                                            now_callback, text_callback, \
                                            logger) \
    { \
        .registry = (registry_value), .callback_context = (context_value), \
        .now = (now_callback), .format_text = (text_callback), .log = (logger) \
    }
...
frame_pacer_vulkan_hud_take_device_swapchains_locked(
    ...
    struct frame_pacer_vulkan_device *device);
FRAME_PACER_VULKAN_INTERNAL void
frame_pacer_vulkan_hud_destroy_swapchain_list(
