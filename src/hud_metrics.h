...
/* Single-owner sampler. The HUD cache worker owns this state and publishes
 * snapshots under its separate cache lock. Join the worker before destruction.
 * Standalone callers must likewise serialize access and lifetime. */
...
frame_pacer_metrics_reset_utilization(struct frame_pacer_metrics *);
...
void frame_pacer_metrics_test_set_gpu_identity(struct frame_pacer_metrics *,
                                               const char *render_node,
                                               unsigned int vendor,
                                               const char *pci_bus_id);
bool frame_pacer_metrics_test_find_amd_temperature(struct frame_pacer_metrics *,
                                                   const char *drm_root);
#endif
