...
static bool parse_temperature(const char *text, unsigned int *celsius)
{
    ...
    while (*cursor && isspace((unsigned char)*cursor))
        ++cursor;
    if (*cursor || millidegrees > UINT64_C(200000))
        return false;
...
bool frame_pacer_metrics_parse_cpu(const char *line, uint64_t *total,
                                   uint64_t *idle)
{
    uint64_t values[8];
    ...
    if (!line || !total || !idle || strncmp(line, "cpu", 3))
        return false;
    cursor = line + 3;
    while (isdigit((unsigned char)*cursor))
        ++cursor;
    ...
    for (index = 0; index < sizeof(values) / sizeof(values[0]); ++index) {
        while (*cursor == ' ' || *cursor == '\t')
            ++cursor;
        if (!parse_uint64(&cursor, &values[index]))
            return false;
    ...
    for (;;) {
        ...
        if (!*cursor)
            break;
        if (!parse_uint64(&cursor, &ignored))
            return false;
        if (*cursor && *cursor != ' ' && *cursor != '\t' && *cursor != '\r' &&
            *cursor != '\n')
            return false;
...
static bool thread_cgroup_root(char *output, size_t size)
{
    ...
    if (strncmp(line, "0::/", 4))
        return false;
    match = strstr(line + 3, marker);
    if (!match)
        return false;
    written =
        snprintf(output, size, "/sys/fs/cgroup%.*s/frame-pacer-thread-cpu",
                             ...
                             uint64_t *usage_usec)
{
    ...
    while (fgets(line, sizeof(line), file))
        if (frame_pacer_metrics_parse_cpu_stat(line, "usage_usec",
                                               usage_usec)) {
            (void)fclose(file);
            return true;
        }
...
static struct frame_pacer_thread_cpu_sample *
thread_cpu_slot(struct frame_pacer_metrics *metrics, uint32_t tid)
{
    unsigned int i;
    ...
    for (i = 0; i < FRAME_PACER_THREAD_CPU_SLOTS_MAX; ++i)
        if (!metrics->thread_cpu[i].started)
            return &metrics->thread_cpu[i];
    return 0;
}
                                   ...
                                   uint64_t sample_ns)
{
    ...
    for (index = 0; index < FRAME_PACER_THREAD_CPU_SLOTS_MAX; ++index) {
        struct frame_pacer_thread_cpu_sample *sample =
            &metrics->thread_cpu[index];
                              ...
                              struct frame_pacer_metrics_snapshot *snapshot)
{
    ...
    if (!now_ns)
        goto cached;
    if (!thread_cgroup_root(root, sizeof(root))) {
    ...
    if (metrics->thread_cpu_sample_ns &&
        now_ns - metrics->thread_cpu_sample_ns <
            THREAD_CPU_SAMPLE_INTERVAL_NS) {
    ...
    while ((entry = readdir(directory))) {
        ...
        if (strncmp(entry->d_name, "t-", 2))
            continue;
        value = strtoul(entry->d_name + 2, &end, 10);
...
bool frame_pacer_metrics_parse_render_node(const char *target, char *node,
                                           size_t node_size)
{
    const char *base;
    int written;
...
static bool parse_pci_bus_id(const char *target, char *pci, size_t size)
{
    ...
    for (index = 0; index < sizeof(hex_positions) / sizeof(hex_positions[0]);
         ++index)
        if (!isxdigit((unsigned char)base[hex_positions[index]]))
            return false;
    memcpy(pci, base, 13);
    return true;
}
...
static void find_gpu_identity(struct frame_pacer_metrics *metrics)
{
    ...
    if (length < 0)
        return;
    target[length] = '\0';
    (void)parse_pci_bus_id(target, metrics->gpu_pci_bus_id,
                           sizeof(metrics->gpu_pci_bus_id));
}
    ...
    unsigned int vendor, const char *pci_bus_id)
{
    if (!metrics)
        return;
    (void)snprintf(metrics->gpu_render_node, sizeof(metrics->gpu_render_node),
                   "%s", render_node ? render_node : "");
    (void)snprintf(metrics->gpu_pci_bus_id, sizeof(metrics->gpu_pci_bus_id),
                   "%s", pci_bus_id ? pci_bus_id : "");
                                   ...
                                   const char *path)
{
    ...
    if (access(path, R_OK))
        return false;
    written = snprintf(metrics->gpu_temp_path, sizeof(metrics->gpu_temp_path),
                       "%s", path);
                                    ...
                                    const char *drm_root)
{
    ...
    if (!metrics || !drm_root || !*drm_root)
        return false;
    metrics->gpu_temp_path[0] = '\0';
    if (metrics->gpu_vendor != 0x1002U || !metrics->gpu_render_node[0])
        return false;
    written =
        snprintf(directory_path, sizeof(directory_path), "%s/%s/device/hwmon",
                 drm_root, metrics->gpu_render_node);
    ...
    while ((entry = readdir(directory))) {
        ...
        if (entry->d_name[0] == '.')
            continue;
        written = snprintf(path, sizeof(path), "%s/%s/name", directory_path,
                           entry->d_name);
        if (written < 0 || (size_t)written >= sizeof(path) ||
...
static void find_process_gpu(struct frame_pacer_metrics *metrics,
                             unsigned int process_id)
{
    ...
    written = snprintf(directory_path, sizeof(directory_path), "/proc/%u/fd",
                       process_id);
    if (!process_id || written < 0 || (size_t)written >= sizeof(directory_path))
        return;
    directory = opendir(directory_path);
    if (!directory)
        return;
    ...
    if (metrics->gpu_vendor != 0x10deU && fallback_node[0]) {
        (void)snprintf(metrics->gpu_render_node,
                       sizeof(metrics->gpu_render_node), "%s", fallback_node);
        (void)snprintf(metrics->gpu_pci_bus_id, sizeof(metrics->gpu_pci_bus_id),
                       "%s", fallback_pci);
...
static void find_cpu_temperature(struct frame_pacer_metrics *metrics)
{
    ...
    while ((entry = readdir(directory))) {
        ...
        if (written < 0 || (size_t)written >= sizeof(path))
            continue;
        if (!read_trimmed_line(path, name, sizeof(name)))
            continue;
        if (strcmp(name, "coretemp"))
            continue;
        for (index = 1; index <= 16; ++index) {
            written =
                snprintf(path, sizeof(path), "/sys/class/hwmon/%s/temp%d_label",
                         entry->d_name, index);
            if (written < 0 || (size_t)written >= sizeof(path))
                break;
            if (!read_trimmed_line(path, label, sizeof(label)))
                continue;
            if (!strcmp(label, "Package id 0")) {
                written = snprintf(
                    metrics->cpu_temp_path, sizeof(metrics->cpu_temp_path),
                    "/sys/class/hwmon/%s/temp%d_input", entry->d_name, index);
                if (written >= 0 &&
                                ...
                                struct frame_pacer_metrics_snapshot *snapshot)
{
    ...
    file = metrics->cpu_temp_path[0] ? fopen(metrics->cpu_temp_path, "re") : 0;
    if (file) {
        if (fgets(line, sizeof(line), file) &&
    ...
    file = !(snapshot->available & FRAME_PACER_METRIC_GPU_TEMP) &&
                   metrics->gpu_temp_path[0]
               ? fopen(metrics->gpu_temp_path, "re")
               : 0;
    if (file) {
        if (fgets(line, sizeof(line), file) &&
    ...
    if (!(snapshot->available & FRAME_PACER_METRIC_GPU_USE) &&
        ...
        frame_pacer_drm_fdinfo_sample(&metrics->drm_fdinfo, metrics->process_id,
