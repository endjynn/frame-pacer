...
bool frame_pacer_thread_cpu_quota_test_write_text(const char *path,
                                                  const char *text)
{
    return write_text(path, text);
}
#endif
static void record_failure(struct frame_pacer_thread_cpu_quota *q,
                           const char *stage, int error)
{
    ...
    if (!q || !stage)
        return;
    if (q->last_error == error && !strcmp(q->failure_stage, stage))
        return;
    q->last_error = error;
    (void)snprintf(q->failure_stage, sizeof(q->failure_stage), "%s", stage);
    (void)snprintf(message, sizeof(message),
                   "frame-pacer: thread CPU ceiling %s: %s\n", stage,
                   error ? strerror(error) : "unavailable");
...
static void record_state(struct frame_pacer_thread_cpu_quota *q,
                         const char *stage)
{
    char message[128];
    void (*log)(const char *);
    ...
    if (!q || !stage)
        return;
    if (!strcmp(q->failure_stage, stage) && q->last_error == INT_MIN)
        return;
    q->last_error = INT_MIN;
    (void)snprintf(q->failure_stage, sizeof(q->failure_stage), "%s", stage);
    (void)snprintf(message, sizeof(message),
                   "frame-pacer: thread CPU ceiling %s\n", stage);
    (void)pthread_mutex_lock(&q->mutex);
    log = q->log;
    (void)pthread_mutex_unlock(&q->mutex);
    if (log)
...
static bool join_path(char *out, size_t n, const char *base, const char *child,
                      const char *file)
{
    int written =
        snprintf(out, n, "%s%s%s%s%s", base, child ? "/" : "",
...
static bool valid_boot_id(const char *boot)
{
    ...
    for (index = 0; index < 36; ++index) {
        ...
        unsigned char digit = (unsigned char)boot[index];
...
static bool identity(char *scope, size_t n)
{
    ...
    (void)fclose(file);
    char *ending = strpbrk(boot, "\r\n");
    if (ending)
        *ending = '\0';
    if (!valid_boot_id(boot))
        return false;
    written =
        snprintf(scope, n, "frame-pacer-thread-cpu-u%ju-b%.12s-p%ju.scope",
...
static bool cgroup_root(char *out, size_t n)
{
    ...
    for (i = 0; i < 24 && pid > 1; ++i) {
        ...
        written =
            snprintf(status, sizeof(status), "/proc/%ld/status", (long)pid);
        ...
        while (fgets(line, sizeof(line), file)) {
            ...
            if (sscanf(line, "PPid:\t%lu", &value) == 1)
                parent = (pid_t)value;
        }
        (void)fclose(file);
        written = snprintf(mountinfo, sizeof(mountinfo), "/proc/%ld/mountinfo",
                           (long)pid);
        if (written < 0 || (size_t)written >= sizeof(mountinfo))
            return false;
        if (mount_is_rw(mountinfo)) {
            written =
                snprintf(out, n, "/proc/%ld/root/sys/fs/cgroup", (long)pid);
                       ...
                       const char *scope)
{
    ...
    for (i = 0; i < 50; ++i) {
        ...
        if (file && fgets(line, sizeof(line), file) &&
            ...
            char *ending = strpbrk(path, "\r\n");
            if (ending)
                *ending = '\0';
            terminal = strrchr(path, '/');
            terminal = terminal ? terminal + 1 : path;
            scope_length =
                snprintf(q->scope, sizeof(q->scope), "%s%s", root, path);
             ...
             * identity and a numeric owned child; this joins that controller
             * and never claims a sibling hierarchy. */
            {
                ...
                if (owned) {
                    ...
                    scope_length = snprintf(q->scope, sizeof(q->scope), "%s%s",
                                            root, path);
                    proc_length =
                        snprintf(q->cgroup_proc, sizeof(q->cgroup_proc),
                                 "%s/frame-pacer-thread-cpu", path);
...
static uint32_t collect(uint32_t out[FRAME_PACER_THREAD_CPU_QUOTA_TIDS_MAX],
                        bool *overflow)
{
...
static bool tid_path_is(uint32_t tid, const char *expected)
{
    ...
    FILE *file;
    int written =
        snprintf(path, sizeof(path), "/proc/self/task/%u/cgroup", tid);
...
static void cleanup(struct frame_pacer_thread_cpu_quota *q)
{
    ...
    if (q->owner && q->cgroup[0]) {
        ...
        for (i = 0; i < count; ++i) {
            ...
            if (!tid_in(q->tids, q->observed_threads, now[i]))
                continue;
...
static bool activate(struct frame_pacer_thread_cpu_quota *q, uint32_t quota)
{
    ...
    if (mkdir(q->cgroup, 0700)) {
        record_failure(
            q, errno == EEXIST ? "claim threaded root" : "create threaded root",
            errno);
        return false;
    }
    q->owner = true;
    ...
    if (!join_path(path, sizeof(path), q->cgroup, 0, "cgroup.type") ||
        ...
        !join_path(path, sizeof(path), q->scope, 0, "cgroup.subtree_control") ||
        !write_text(path, "+cpu") ||
...
static enum reconcile_result reconcile(struct frame_pacer_thread_cpu_quota *q,
                                       uint32_t quota)
{
    ...
    written =
        snprintf(wanted, sizeof(wanted), "%u %u", quota * 1000U, CPU_PERIOD);
    ...
    for (i = 0; i < count; ++i) {
        ...
        if (!join_path(path, sizeof(path), q->cgroup, child, "cgroup.threads"))
            return RECONCILE_FATAL;
...
static void *worker(void *arg)
{
    ...
    for (;;) {
        ...
        if (!active)
            active = activate(q, wanted);
        if (active && q->external) {
            bool confirmed;

            if (last != wanted && !frame_pacer_thread_cpu_external_write(
                                      q->external_state, true, wanted)) {
                (void)pthread_mutex_lock(&q->mutex);
                q->confirmed = false;
                (void)pthread_mutex_unlock(&q->mutex);
            } else if (last != wanted) {
            ...
            (void)pthread_mutex_lock(&q->mutex);
            q->confirmed =
                confirmed && q->requested_enabled && q->requested == wanted;
            (void)pthread_mutex_unlock(&q->mutex);
            if (!confirmed)
                active = false;
        } else if (active) {
            ...
            if (result == RECONCILE_CONFIRMED) {
                last = wanted;
                (void)pthread_mutex_lock(&q->mutex);
                q->confirmed = q->requested_enabled && q->requested == wanted;
                (void)pthread_mutex_unlock(&q->mutex);
            } else {
                (void)pthread_mutex_lock(&q->mutex);
                q->confirmed = false;
...
void frame_pacer_thread_cpu_quota_init(struct frame_pacer_thread_cpu_quota *q)
{
    ...
    q->initialized = true;
    if (!pthread_create(&q->worker, 0, worker, q))
        q->worker_started = true;
}
void frame_pacer_thread_cpu_quota_destroy(
    struct frame_pacer_thread_cpu_quota *q)
{
    if (!q || !q->initialized)
        return;
    (void)pthread_mutex_lock(&q->mutex);
...
void frame_pacer_thread_cpu_quota_set_logger(
    struct frame_pacer_thread_cpu_quota *q, void (*log)(const char *))
{
    if (!q || !q->initialized)
        return;
    (void)pthread_mutex_lock(&q->mutex);
