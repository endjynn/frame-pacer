...
static bool valid(const struct frame_pacer_hud_record_provider *p)
{
    return p && p->reset_command_buffer && p->begin_command_buffer &&
        p->end_command_buffer && p->pipeline_barrier && p->begin_render_pass &&
        p->end_render_pass && p->bind_pipeline && p->bind_vertex_buffers &&
        p->draw && p->set_viewport && p->set_scissor && p->push_constants;
}

bool frame_pacer_hud_record(const struct frame_pacer_hud_record_provider *p,
    VkCommandBuffer command, VkImage image, VkFramebuffer framebuffer,
    VkRenderPass pass, const struct frame_pacer_hud_pipeline *pipeline,
    ...
    uint32_t vertex_count)
{
    ...
    const VkRenderPassBeginInfo render = {
        ...
        .framebuffer = framebuffer, .renderArea = {.extent = extent},
    };
    const VkViewport viewport = {.width = (float)extent.width, .height = (float)extent.height,
                                 .minDepth = 0.0f, .maxDepth = 1.0f};
    const VkRect2D scissor = {.extent = extent};
    const VkDeviceSize offset = 0;
    const float inverse_extent[2] = {1.0f / (float)extent.width, 1.0f / (float)extent.height};

    if (!valid(p) || !command || !image || !framebuffer || !pass || !pipeline ||
        !pipeline->pipeline || !pipeline->layout || !vertices || !vertices->buffer ||
        !extent.width || !extent.height || !vertex_count)
        return false;
    ...
    p->pipeline_barrier(command, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
        VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, 0, 0, 0, 0, 0, 1, &to_color);
    p->begin_render_pass(command, &render, VK_SUBPASS_CONTENTS_INLINE);
    p->bind_pipeline(command, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline->pipeline);
    ...
    p->pipeline_barrier(command, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT,
        VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, 0, 0, 0, 0, 0, 1, &to_present);
