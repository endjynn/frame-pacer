...
#define REPORT_FIXED_MAX                                                       \
    (LITERAL_BYTES("frame-pacer: effective-config revision=") + 20U +          \
     LITERAL_BYTES(" trigger=") + 7U + LITERAL_BYTES(" backend=") + 6U +       \
     LITERAL_BYTES(" renderer=") + 2U + LITERAL_BYTES(" config=") + 10U +      \
     LITERAL_BYTES(" rule=") + 2U + LITERAL_BYTES(" match=") + 2U +            \
     LITERAL_BYTES(" fps=") + 3U + LITERAL_BYTES(" fps_source=") + 8U +        \
     LITERAL_BYTES(" hud=") + 3U + LITERAL_BYTES(" hud_source=") + 8U +        \
     LITERAL_BYTES(" thread_cpu=") + 4U +                                      \
     LITERAL_BYTES(" thread_cpu_source=") + 8U + LITERAL_BYTES(" reason=") +   \
     23U + LITERAL_BYTES(" line=") + 20U + 1U)

_Static_assert(
    REPORT_FIXED_MAX + 3U * ENCODED_TEXT_LIMIT <=
...
{
    switch (backend) {
    case FRAME_PACER_REPORT_VULKAN:
        return "vulkan";
    case FRAME_PACER_REPORT_GLX:
        return "glx";
    case FRAME_PACER_REPORT_EGL:
        return "egl";
    case FRAME_PACER_REPORT_BACKEND_COUNT:
        break;
    }
    return 0;
}
...
{
    switch (status) {
    case FRAME_PACER_CONFIG_VALID:
        return "valid";
    case FRAME_PACER_CONFIG_MISSING:
        return "missing";
    case FRAME_PACER_CONFIG_INSECURE:
        return "insecure";
    case FRAME_PACER_CONFIG_UNREADABLE:
        return "unreadable";
    case FRAME_PACER_CONFIG_MALFORMED:
        return "malformed";
    }
    return 0;
}
...
{
    switch (source) {
    case FRAME_PACER_SOURCE_DEFAULT:
        return "default";
    case FRAME_PACER_SOURCE_GLOBAL:
        return "global";
    case FRAME_PACER_SOURCE_PER_GAME:
        return "per-game";
    }
    return 0;
}
...
static void encode_text(const char *input, char output[ENCODED_TEXT_CAPACITY])
{
    ...
    while (input[input_offset]) {
        char encoded[4];
        size_t bytes =
            escaped_byte((unsigned char)input[input_offset], encoded);
    ...
    enum frame_pacer_report_backend backend, bool startup)
{
    ...
    char match[ENCODED_TEXT_CAPACITY],
        renderer_field[ENCODED_TEXT_CAPACITY + 2U];
    char rule_field[ENCODED_TEXT_CAPACITY + 2U],
        match_field[ENCODED_TEXT_CAPACITY + 2U];
    ...
    const char *thread_source =
        config ? source_name(config->thread_cpu_source) : 0;
    int written;

    if (!output || !capacity || !config || !config->revision || !backend_text ||
        !status || !reason || !fps_source || !hud_source || !thread_source)
        return 0;
    ...
    if (config->thread_cpu_enabled)
        (void)snprintf(thread_cpu, sizeof(thread_cpu), "%u%%",
                       config->thread_cpu_percent);
    else
    ...
    written =
        snprintf(output, capacity,
                 ...
                 renderer_field, status, rule_field, match_field, fps,
                 fps_source, config->hud_enabled ? "on" : "off", hud_source,
...
bool frame_pacer_effective_report_if_due(
    ...
    struct frame_pacer_limit *limit, enum frame_pacer_report_backend backend,
    frame_pacer_report_write_fn write_message, void *context)
{
    ...
    if (!reporter || !limit || !write_message ||
        (unsigned int)backend >= FRAME_PACER_REPORT_BACKEND_COUNT)
        return false;
    revision = frame_pacer_limit_revision(limit);
    reported = atomic_load_explicit(&reporter->reported[backend],
                                    memory_order_acquire);
    if (!revision || revision == reported)
        return false;
    (void)pthread_mutex_lock(&reporter->mutex);
    reported = atomic_load_explicit(&reporter->reported[backend],
                                    memory_order_relaxed);
    if (!frame_pacer_limit_snapshot(limit, &snapshot) ||
        snapshot.revision == reported) {
        (void)pthread_mutex_unlock(&reporter->mutex);
        return false;
    }
    startup = reported == 0;
    if (!frame_pacer_effective_report_format(message, sizeof(message),
                                             &snapshot, backend, startup)) {
        (void)pthread_mutex_unlock(&reporter->mutex);
        return false;
    }
