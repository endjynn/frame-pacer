                                    ...
                                    const char *library)
{
    ...
    if (!provider->library) return false;
...
void frame_pacer_nvml_provider_destroy(struct frame_pacer_nvml_provider *provider)
{
    if (!provider) return;
    if (provider->started) (void)provider->shutdown();
    if (provider->library) (void)dlclose(provider->library);
    memset(provider, 0, sizeof(*provider));
}
    ...
    struct frame_pacer_nvml_provider *provider, unsigned int process_id)
{
    ...
    if (!provider || !provider->started || !provider->get_count ||
        ...
        !process_id ||
        provider->get_count(&count) != 0 || count > NVML_MAX_DEVICES)
        return false;
    provider->device = 0;
    for (index = 0; index < count; ++index) {
        ...
        if (provider->get_device(index, &device) != 0 || !device) continue;
        ...
        free(processes);
        if (provider->device) return true;
    }
    return false;
}

bool frame_pacer_nvml_provider_select_pci(struct frame_pacer_nvml_provider *provider,
...
bool frame_pacer_nvml_provider_sample(struct frame_pacer_nvml_provider *provider,
                                      struct frame_pacer_nvml_sample *sample)
{
    ...
    if (!provider || !provider->started || !provider->device) return false;
    ...
    if (provider->temperature(provider->device, 0,
                              &sample->gpu_temp_celsius) == 0 &&
