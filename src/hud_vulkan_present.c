...
bool frame_pacer_hud_prepare_present(
    const struct frame_pacer_hud_present_provider *p, VkDevice device,
    VkQueue queue, const VkPresentInfoKHR *original, VkFence fence,
    const VkSemaphore *signal, VkCommandBuffer command, uint32_t image_index,
    uint32_t image_count, frame_pacer_hud_record_fn record, void *context,
    VkPresentInfoKHR *replacement)
{
    ...
    if (!p || !p->wait_for_fences || !p->reset_fences || !p->queue_submit ||
        !original || !fence || !signal || !*signal || !command || !record ||
        !replacement || image_index >= image_count ||
        original->swapchainCount != 1 ||
    ...
    for (wait_index = 0; wait_index < original->waitSemaphoreCount;
         ++wait_index)
        stages[wait_index] = VK_PIPELINE_STAGE_ALL_COMMANDS_BIT;
    submit.pWaitDstStageMask = original->waitSemaphoreCount ? stages : 0;
    if (p->wait_for_fences(device, 1, &fence, VK_TRUE, UINT64_MAX) !=
            VK_SUCCESS ||
        p->reset_fences(device, 1, &fence) != VK_SUCCESS ||
        !record(context, image_index) ||
